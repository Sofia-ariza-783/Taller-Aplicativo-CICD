...
/**
 * Service class for managing Participant entities. Handles business logic for participant-related
 * operations including creation, retrieval, and deletion.
 */
@Service
@AllArgsConstructor
public class ParticipantService {
   ...
   * @throws RuntimeException if a participant with the same name already exists or if there's an
   *     error during creation
   */
  public Participant createParticipant(ParticipantDto participantDto) {
    Participant existingParticipant =
        participantRepository.findByFullName(participantDto.fullName());
    if (existingParticipant != null) {
      logger.error("Participant with name {} already exists", participantDto.fullName());
      throw new RuntimeException(
          "Participant with name " + participantDto.fullName() + " already exists");
    }

    String id = UUID.randomUUID().toString();
    Participant participant =
        new Participant(id, participantDto.fullName(), participantDto.season());
    try {
