 ...
 * Handles business logic for recipe-related operations including creation, retrieval, updating, and deletion.
 */
@Service
@AllArgsConstructor
public class RecipeService {
    ...
    public Recipe createRecipe(RecipeDto recipeDto) {
        ...
        List<String> ingredients = Arrays.stream(recipeDto.ingredients().split(",")).map(String::trim).toList();
        List<String> instructions = Arrays.stream(recipeDto.instructions().split(";")).map(String::trim).toList();
    ...
    /**
     * Retrieves a recipe by the author's name.
     * The author can be either a Chef or a Viewer.
     *
    ...
    public Recipe getRecipeByNumber(int number) {
        return recipeRepository.findByNum(number)
                .orElseThrow(() -> {
    ...
    public Recipe updateRecipe(String id, RecipeDto recipeDto){
        ...
        if (currentRecipe.isEmpty()) {
            ...
            throw new RuntimeException("Recipe with author name " + recipeDto.author() + " does not exists");
        }
