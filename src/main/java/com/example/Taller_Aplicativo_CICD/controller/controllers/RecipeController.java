 ...
 * Provides endpoints for creating, retrieving, and managing recipes.
 */
@RestController
...
public class RecipeController {
     ...
     * @return ResponseEntity containing the created Recipe and HTTP status 201 if successful,
     ...
     * @return ResponseEntity containing the RecipeResponseDto if found (HTTP 200),
     *         or HTTP status 404 if not found
     */
    @GetMapping("/participant/{authorName}")
    @Operation(summary = "Get recipe by participant name", description = "Retrieves a recipe along with the participant's season")
     ...
     * @return ResponseEntity containing the Recipe if found (HTTP 200),
     *         or HTTP status 404 if not found
     */
    @GetMapping("/chef/{chefName}")
    @Operation(summary = "Get recipe by chef name", description = "Retrieves a recipe by the chef's name")
    ...
    @Operation(summary = "Get recipe by viewer name", description = "Retrieves a recipe by the viewer's name")
     ...
     *
     * @return ResponseEntity containing a list of all Recipes (HTTP 200),
     *         or HTTP status 500 if there's a server error
     */
    @GetMapping
    ...
    @Operation(summary = "Get recipe by number", description = "Retrieves a recipe by its unique number")
     ...
     * @return ResponseEntity containing a list of RecipeResponseDto for the specified season (HTTP 200),
     *         or HTTP status 404 if no recipes found for the season
     */
    @GetMapping("/season/{season}")
    @Operation(summary = "Get recipes by season", description = "Retrieves all recipes for a specific season")
     ...
     * @return ResponseEntity containing a list of Recipes containing the ingredient (HTTP 200),
     *         or HTTP status 404 if no recipes found with the ingredient
     */
    @GetMapping("/ingredient/{ingredient}")
    @Operation(summary = "Get recipes by ingredient", description = "Retrieves all recipes containing a specific ingredient")
     ...
     * @return ResponseEntity containing the updated Recipe (HTTP 200),
     *         or HTTP status 404 if the recipe doesn't exist
     */
    @PutMapping("/{id}")
    @Operation(summary = "Update a recipe", description = "Updates an existing recipe with new information")
    public ResponseEntity<Recipe> updateRecipe(@PathVariable String id, @RequestBody RecipeDto recipeDto) {
        try {
