...
/**
 * REST controller for managing Viewer entities. Provides endpoints for creating, retrieving, and
 * deleting viewers.
 */
@RestController
...
public class ViewerController {
   ...
   * @return ResponseEntity containing the created viewer and HTTP status 201 if successful, or HTTP
   ...
   * @return ResponseEntity containing the viewer if found (HTTP 200), or HTTP status 404 if no
   *     viewer with the given name exists
   */
  @GetMapping("/{fullName}")
   ...
   *     Found) if the viewer doesn't exist
   */
  @DeleteMapping("/{id}")
